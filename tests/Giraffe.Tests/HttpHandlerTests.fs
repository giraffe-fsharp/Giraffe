...
let ``GET "/json" returns json object with fsharp type (JsonUnionCaseDummyA)`` () =
    ...
    let expected =
        """{"ValueA":"hello","ValueB":{"Case":"JsonUnionCaseDummyA","Fields":[42]}}"""
...
let ``GET "/json" returns json object with fsharp type and use custom config`` () =
    ...
    let customConfig =
        System.Text.Json.Serialization.JsonFSharpOptions
            .Default()
...
let ``GET "/jsonChunked" returns json object`` (size: int) =
    ...
    let app =
        ...
        >=> choose [
            ...
            route "/foo" >=> text "bar"
            route "/jsonChunked"
            >=> json (Array.replicate size { Foo = "john"; Bar = "doe"; Age = 30 })
            setStatusCode 404 >=> text "Not found"
        ]
...
let ``POST "/post/1" returns "1"`` () =
    ...
    let app =
        choose [
            GET >=> choose [ route "/" >=> text "Hello World"; route "/foo" >=> text "bar" ]
            POST >=> choose [ route "/post/1" >=> text "1"; route "/post/2" >=> text "2" ]
            setStatusCode 404 >=> text "Not found"
        ]
    ...
    let app =
        choose [
            ...
            >=> choose [
                route "/any" >=> mustAccept [ "text/plain" ] >=> text "first route"
                route "/any"
                >=> mustAccept [ "application/xml" ]
                >=> text "<test>second route</test>"
                route "/any"
                >=> mustAccept [ "text/plain"; "application/json" ]
                >=> text "third route"
            ]
...
let ``GET "/person" returns rendered HTML view`` () =
    ...
    let app =
        choose [
            ...
            POST >=> choose [ route "/post/1" >=> text "1" ]
            setStatusCode 404 >=> text "Not found"
        ]
    ...
    let expected =
        "<!DOCTYPE html><html><head><title>Html Node</title></head><body><p>John Doe is 30 years old.</p></body></html>"
...
let johnDoeAsText =
    @"First name: John, Last name: Doe, Birth date: 1990-07-12, Height: 1.85, Piercings: [|""ear""; ""nose""|]"
...
let JsonReturningAcceptHeaderCases =
    [
        {
            NegotiationConfig = JsonOnlyNegotiationConfig() :> INegotiationConfig
            StatusCode = 200
            ReturnContentType = "application/json; charset=utf-8"
        }
        {
            NegotiationConfig = DefaultNegotiationConfig() :> INegotiationConfig
            StatusCode = 200
            ReturnContentType = "application/json; charset=utf-8"
        }
    ]
...
let ``Get "/auto" with Accept header of "application/json" returns JSON object``
    (config: NegotiationConfigWithExpectedResult)
    =
    let ctx =
        getNegotiationTestHttpContext config.NegotiationConfig false (StringValues("application/json"))
...
let ``Get "/auto" with Accept header of "application/xml; q=0.9, application/json" returns JSON object``
    ...
    =
    let ctx =
        getNegotiationTestHttpContext
            config.NegotiationConfig
            false
            (StringValues("application/xml; q=0.9, application/json"))
...
let XmlReturningAcceptHeaderCases =
    [
        ...
        }
        {
            NegotiationConfig = DefaultNegotiationConfig() :> INegotiationConfig
            StatusCode = 200
            ReturnContentType = "application/xml; charset=utf-8"
        }
    ]
    |> toTheoryData
...
let ``Get "/auto" with Accept header of "application/xml" returns XML object``
    ...
    =
    let ctx =
        getNegotiationTestHttpContext config.NegotiationConfig true (StringValues("application/xml"))
...
let ``Get "/auto" with Accept header of "application/xml, application/json" returns XML object``
    ...
    =
    let ctx =
        getNegotiationTestHttpContext config.NegotiationConfig true (StringValues("application/xml, application/json"))
    ...
    let testChecks (context: HttpContext) =
        ...
        if
            ...
            && config.ReturnContentType = "application/xml; charset=utf-8"
        then
            let body = getBody context
            XmlAssert.equals johnDoeAsXml body
            Assert.Equal(config.ReturnContentType, context.Response |> getContentType)
...
let ``Get "/auto" with Accept header of "application/json, application/xml" returns JSON object``
    ...
    =
    let ctx =
        getNegotiationTestHttpContext config.NegotiationConfig false (StringValues("application/json, application/xml"))
...
let ``Get "/auto" with Accept header of "application/json; q=0.5, application/xml" returns XML object``
    ...
    let ctx =
        getNegotiationTestHttpContext
            config.NegotiationConfig
            true
            (StringValues("application/json; q=0.5, application/xml"))
...
let ``Get "/auto" with Accept header of "application/json; q=0.5, application/xml; q=0.6" returns XML object``
    ...
    let ctx =
        getNegotiationTestHttpContext
            config.NegotiationConfig
            true
            (StringValues("application/json; q=0.5, application/xml; q=0.6"))
...
let TextReturningAcceptHeaderCases =
    [
        ...
        }
        {
            NegotiationConfig = DefaultNegotiationConfig() :> INegotiationConfig
            StatusCode = 200
            ReturnContentType = "text/plain; charset=utf-8"
        }
    ]
    |> toTheoryData
...
let ``Get "/auto" with Accept header of "text/plain; q=0.7, application/xml; q=0.6" returns text object``
    ...
    let ctx =
        getNegotiationTestHttpContext
            config.NegotiationConfig
            false
            (StringValues("text/plain; q=0.7, application/xml; q=0.6"))
...
let ``Get "/auto" with Accept header of "text/html" returns a 406 response``
    ...
    =
    let ctx =
        getNegotiationTestHttpContext config.NegotiationConfig false (StringValues("text/html"))
...
let ``Get "/auto" without an Accept header returns a JSON object`` (config: NegotiationConfigWithExpectedResult) =
    let ctx =
        getNegotiationTestHttpContext config.NegotiationConfig false StringValues.Empty
