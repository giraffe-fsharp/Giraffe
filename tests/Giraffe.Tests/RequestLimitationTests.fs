...
module private Helpers =
  ...
  let private customInvalidHeaderValueHandler (header : string) =
    RequestErrors.notAcceptable (json {| Message = $"{header} header is not valid" |})

  let private customHeaderNotFoundHandler (header : string) =
    RequestErrors.notAcceptable (json {| Message = $"{header} header was not found" |})
...
let ``block: request with no 'Accept' header`` () =
  ...
  let app = GET >=> mustAcceptAny [ "application/json" ] optionalErrorHandler 
                >=> setStatusCode 200 
                >=> text "allowed"
...
let ``block: request with unallowed 'Accept' header`` () =
  ...
  ctx.Response.Body <- new MemoryStream()
  let expected = "Request rejected because 'Accept' header hasn't got expected MIME type"
...
let ``block: request with no 'Content-Type' header`` () =
  ...
  let app = GET >=> hasContentType "application/json" optionalErrorHandler 
                >=> setStatusCode 200 
                >=> text "allowed"
...
let ``block: request with unallowed 'Content-Type' header`` () =
  ...
  ctx.Response.Body <- new MemoryStream()
  let expected = "Request rejected because 'Content-Type' header hasn't got expected value"
...
let ``block: request without 'Content-Length' header`` () =
  ...
  let app = GET >=> maxContentLength 1000L optionalErrorHandler 
                >=> setStatusCode 200 
                >=> text "allowed"
...
let ``block: request with exceeded 'Content-Length' header`` () =
  ...
  let app = GET >=> maxContentLength 1L optionalErrorHandler 
                >=> setStatusCode 200 
                >=> text "allowed"
